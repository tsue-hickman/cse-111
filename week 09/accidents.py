...
def main():
    try:
        ...
        with open(filename, "rt") as text_file:
            ...
            perc_reduc = get_valid_percentage()
            ...
            for row in reader:
                ...
                try:
                    ...
                    print(f"Error: {filename}, line {reader.line_num}: {ex}")
                    continue
...
def get_valid_percentage():
    """
    Prompts the user for a percentage value and returns a valid percentage.
    """
    while True:
        try:
            perc_reduc = float(input("Percent reduction of texting while driving [0, 100]: "))
            if perc_reduc < 0 or perc_reduc > 100:
                print(f"Error: {perc_reduc} is out of range. Please enter a different number.")
            else:
                return perc_reduc
        except ValueError as ex:
            print(f"Error: could not convert string to float: '{ex.args[0]}'")
...
def estimate_reduction(row, behavior_key, perc_reduc):
    ...
    if fatal_crashes == 0:
        raise ZeroDivisionError("Cannot divide by zero in 'Fatal Crashes' column")
